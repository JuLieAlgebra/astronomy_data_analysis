...
def plot_correlation_matrix(cov_m, std, classes, normalize=False, title='Correlation Matrix', cmap=plt.cm.Blues):
    """
...
def plot_galaxies_by_color_indices(data, labels, features, name='Default.png'):
    ...
    for ind, subset in enumerate(itertools.combinations(labels, 2)):
        ...
        subplt.scatter(features[subset[0]].values[::batch], features[subset[1]].values[::batch],
                       marker='.')
...
if __name__ == '__main__':
    ...
    path = os.path.join(parent_directory, os.pardir, 'sample_data', 'galaxy_catalogs', '')
